...
import { updateOrderStatus, removeOrder } from "../../../slices/resslice";
...
export default function UpdateStatus() {
  ...
  const handleUpdateStatus = (index, newStatus) => {
    dispatch(updateOrderStatus({ index, newStatus }));
  };
  ...
  if (!orders || orders.length === 0) {
    return <p className="no-orders">No active orders yet.</p>;
  }
                ...
                {["Pending", "In Progress", "Served", "Completed"].map(
                  (s) => (
                    <button
                      ...
                      onClick={() => handleUpdateStatus(index, s)}
                    >
                      {s}
                    </button>
                  )
